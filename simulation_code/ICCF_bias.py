...
'''
The following code block was used to create the bias histograms under the use of
various ICCF parameters. We don't provide the full code for all parameters,
since that is achieved by simply changing one variable in the code. Therefore
we only provide one example, which can be extended for reproduction.
'''
...
for i in range(1000):
    ...
    result = peakcent(time, cont, time, line, -0.3 * max(time), 0.5 * max(time), 1/24)
    if result[3] == 1:
        received_lags.append(result[2])
    print(i/1000, '%')
    np.savez_compressed(f'variation_1\\result_{i}', ccf = result[4][0], tau = result[4][1])
received_lags = np.array(received_lags)
np.savez_compressed(f'variation_1\\lags', lags = received_lags)

files = glob(f'variation_1\\result_*.npz')

plt.figure(figsize = (12,7))
plt.axvline(50)
plt.title('CCF for rmax = 0.8, t_unit = 1/24')
for path in files:
    data = np.load(path)
    plt.plot(data['tau'], data['ccf'])
plt.xlabel('days')
plt.ylabel('correlation')
plt.show()

data = np.load(f'variation_1\\lags.npz')
lags = data['lags']
fraction = lags / lag - 1
...
plt.figure(figsize = (12,7))
plt.title('Bias histogram for rmax = 0.8, t_unit = 1/24')
...
print('Median fraction, 16th/84th percentile:', np.median(fraction), low, high)
