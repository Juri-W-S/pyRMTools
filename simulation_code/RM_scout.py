...
ax3.plot(t_plot, cont_plot, label = 'continuum curve', color = '#ad0ec3')
ax3.fill_between(t_plot, cont_plot - cont_plot/sn, cont_plot + cont_plot/sn, alpha = 0.3, label = 'continuum curve uncertainty', color = '#ad0ec3')
ax3.plot(t_plot, line_plot, label = 'line curve', color = '#0e48c3')
ax3.fill_between(t_plot, line_plot - line_plot/sn, line_plot + line_plot/sn, alpha = 1, label = 'line curve uncertainty', color = '#aad0f9')
ax3.scatter(t, cont_final, marker = 'x', label = 'continuum data', color = "#b40cff")
ax3.scatter(t, line_final, marker = 'x', label = 'line data', color = '#0c88ff')
...
fraction = received_lags / (lag / (1+z)) -1
low = np.percentile(fraction, 16)
high = np.percentile(fraction, 84)


ax2.hist(fraction, 100, color = '#0e48c3')
ax2.axvline(np.median(fraction), label = 'Median bias', color = '#fc0cff', ls = 'dashed', lw = 2)
ax2.axvline(low, label = '16th percentile', color = '#ff830c', ls = 'dashed', lw = 2)
ax2.axvline(high, label = '84th percentile', color = '#ff830c', ls = 'dashed', lw = 2)
ax2.legend(loc = 'best')
ax2.set_xlabel('bias')
...
ax1.errorbar(luminosity / 1e44, received_lag, yerr = [[lag_error_minus], [lag_error_plus]], label = 'Simulated result', fmt = 'x', elinewidth=1, capsize = 3, color = '#fc0cff')
ax1.scatter(luminosity / 1e44, lag / (1+z), marker = r'$\Delta$', label = 'Expected result', color = '#0e48c3', s = 100)
ax1.plot(luminosities / 1e44, lags, label = 'Reference R-L relation', color = '#0e48c3')
ax1.legend(loc = 'best')
ax1.set_xscale('log')
ax1.set_yscale('log')
ax1.set_xlabel(r'L$_{\rm cont}$ [erg/s]')
ax1.set_ylabel('R [light days]')
ax1.tick_params(which='major', direction='in')
ax1.tick_params(which='minor', direction='in')
ax1.xaxis.set_major_formatter(FuncFormatter(lambda val, pos: rf'$10^{{{int(np.log10(val*1e44))}}}$'))
plt.tight_layout()
plt.show()
plt.close('all')
