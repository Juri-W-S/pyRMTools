...
We provide a step by step installation guide to help installing the MongoDB client and running the pyRMTools database locally with the MongoDB backend.
...
## 4. Using the database with pyRMTools

The database can now be used with pyRMTools by connecting with the `from_mongodb()` class method.
This needs pymongo to be installed in the python enviroment. Depending on the names the user gives
to the database the following command changes, but it is examplatory for the database connection.
...
## 5. Using the database in python outside of pyRMTools
